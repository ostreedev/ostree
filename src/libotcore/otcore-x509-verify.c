...
bool
otcore_x509_init (void)
{
  return true;
}

/* Validate a single x509 signature.  If there is an unexpected state, such
 ...
 */
gboolean
otcore_validate_x509_signature (GBytes *data, GBytes *public_key, GBytes *signature,
                                bool *out_valid, GError **error)
{
  ...
  return TRUE;
#else
  return glnx_throw (error, "x509 signature validation requested, but support not compiled in");
#endif
}
