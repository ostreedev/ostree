...
/* Generate drop-ins for sysroot.mount and boot.mount to preserve them across soft-reboot */
static gboolean
softreboot_mounts_generator (const char *normal_dir, GError **error)
{
  glnx_autofd int normal_dir_dfd = -1;
  ...
  /* Generate drop-in for /sysroot - needed for ostree/bootc commands */
  if (!generate_mount_unit_dropin (normal_dir_dfd, "sysroot.mount", "/sysroot", error))
    return FALSE;

  /* Generate drop-in for /boot - needed for bootc status/switch */
  if (!generate_mount_unit_dropin (normal_dir_dfd, "boot.mount", "/boot", error))
    return FALSE;
                               ...
                               GError **error)
{
  ...
  if (!require_internal_units (normal_dir, early_dir, late_dir, error))
    return FALSE;
  if (!softreboot_mounts_generator (normal_dir, error))
    return FALSE;
