...
uri_and_keyring_to_name (UriAndKeyring *data)
{
  ...
  return g_strdup_printf ("%s+%s", escaped_uri, escaped_keyring);
}
      ...
      for (i = 0; refs[i] != NULL; i++)
        {
          ...
          /* Grab the given ref and a checksum for it from the repo. Don’t bother
           * with #OstreeRepo, since it’s probably preferable to keep the openat()
           * chain going. */
          g_autofree char *checksum = NULL;
          g_autofree gchar *subpath = g_build_filename (refs[i], "refs", "heads", refs[i], NULL);
          gsize checksum_len;
          checksum = glnx_file_get_contents_utf8_at (repos_dfd, subpath, &checksum_len,
                                                     cancellable, &local_error);

          if (checksum == NULL)
            {
              g_debug ("Ignoring ref ‘%s’ on mount ‘%s’ as its repository doesn’t contain the ref: %s",
                       refs[i], mount_name, local_error->message);
              g_clear_error (&local_error);
              continue;
            }
          ...
          keyring = ostree_repo_resolve_keyring_for_ref (parent_repo, refs[i],
                                                         cancellable, &local_error);

          if (keyring == NULL)
            {
              g_debug ("Ignoring ref ‘%s’ on mount ‘%s’ due to missing keyring: %s",
                       refs[i], mount_name, local_error->message);
              g_clear_error (&local_error);
              continue;
            }
          ...
          g_hash_table_insert (supported_ref_to_checksum, (gpointer) refs[i], g_steal_pointer (&checksum));
        }
