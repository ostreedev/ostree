                                        ...
                                        GCancellable *cancellable, GError **error)
{
  ...
  /* Serialize allowlisted bootconfig keys (e.g. ostree-source-tuned).
   * These are custom keys set by consumers that need to survive the staging
   * roundtrip so they are preserved during finalization at shutdown.
   * See allowed_extra_key_prefixes in ostree-bootconfig-parser.c.
   */
  {
    OstreeBootconfigParser *bootconfig = ostree_deployment_get_bootconfig (deployment);
    if (bootconfig)
