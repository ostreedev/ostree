...
is_valid_ref_name (const gchar *ref_name)
{
  ...
  if (!ostree_validate_rev (ref_name, NULL))
    return FALSE;
                 ...
                 gpointer      user_data)
{
   ...
   * I think this has to happen *only* by looking at the local configuration and
   * matching by ref name. There are too many possibilities for attack if we
   * start dynamically matching GPG keyring names across resolver results. */
 ...
 * TODO: Docs
 *
 * FIXME: This will not currently work when installing an app for the first time
 * which is not already in a repository we have configured. We need some root of
 * trust for that case too.
 */
gchar *
ostree_repo_resolve_keyring_for_ref (OstreeRepo    *self,
                                     const gchar   *ref,
                                     GCancellable  *cancellable,
                                     GError       **error)
{
  ...
  g_return_val_if_fail (OSTREE_IS_REPO (self), NULL);
  g_return_val_if_fail (is_valid_ref_name (ref), NULL);
  g_return_val_if_fail (cancellable == NULL || G_IS_CANCELLABLE (cancellable), NULL);
  g_return_val_if_fail (error == NULL || *error == NULL, NULL);

/* TODO: testing hack */
  //return g_strdup ("org.gnome.Hitori-origin.trustedkeys.gpg");

  /* Look through all the currently configured remotes for the given ref. */
  ...
  for (i = 0; remotes[i] != NULL; i++)
    {
      ...
      if (!ostree_repo_remote_list_refs (self, remotes[i], &refs, cancellable, &local_error))
        {
          g_debug ("%s: Failed to list refs for remote ‘%s’: %s",
                   G_STRFUNC, remotes[i], local_error->message);
          continue;
        }

      if (g_hash_table_contains (refs, ref))
        {
          if (keyring_remote == NULL)
            {
              g_debug ("%s: Found match for ref ‘%s’ in remote ‘%s’.",
                       G_STRFUNC, ref, remotes[i]);
            ...
            {
              g_debug ("%s: Duplicate keyring for ref ‘%s’ in remote ‘%s’."
                       "Keyring will be loaded from remote ‘%s’.",
                       G_STRFUNC, ref, remotes[i], keyring_remote->name);
            }
        }
    }
  ...
  if (keyring_remote != NULL)
  ...
  else
    {
      g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
                   "No keyring found configured locally for ref ‘%s’.", ref);
      return NULL;
    }
}
