                    ...
                    GError            **error)
{
  ...
  if (g_variant_n_children (parent_csum) > 0)
    {
      csum_bytes = ostree_checksum_bytes_peek_validate (parent_csum, error);
      if (csum_bytes == NULL)
        goto out;
    }

  if (csum_bytes != NULL && pull_data->maxdepth == -1)
    {
      queue_scan_one_metadata_object_c (pull_data, csum_bytes,
                                        OSTREE_OBJECT_TYPE_COMMIT, recursion_depth + 1);
    }
  else if (csum_bytes != NULL && depth > 0)
    {
      char parent_checksum[65];
      gpointer parent_depthp;
      int parent_depth;

      ostree_checksum_inplace_from_bytes (csum_bytes, parent_checksum);
  ...
  if (!pull_data->is_commit_only)
    {
      ...
      csum_bytes = ostree_checksum_bytes_peek_validate (tree_contents_csum, error);
      if (csum_bytes == NULL)
        goto out;
      ...
      csum_bytes = ostree_checksum_bytes_peek_validate (tree_meta_csum, error);
      if (csum_bytes == NULL)
        goto out;
